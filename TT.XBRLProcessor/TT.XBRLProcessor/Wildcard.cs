namespace TT.XBRLProcessor
{
    ...
    /// <a href="http://beta.c-sharpcorner.com/uploadfile/b81385/efficient-string-matching-algorithm-with-use-of-wildcard-characters/">
    ...
    public static class Wildcard
    {
        ...
        public static bool IsMatch(string input, string pattern, char singleWildcard, char multipleWildcard)
        {
            ...
            bool[][] pointTested = { new bool[input.Length + 1], new bool[pattern.Length + 1] };    // Each true value indicates that input position vs. pattern position has been tested
                ...
                else
                {
                    ...
                    for (int curInputStart = inputPos; curInputStart < input.Length; curInputStart++)
                    {
                        ...
                        if (((curPatternPos == pattern.Length && curInputPos == input.Length) || (curPatternPos < pattern.Length && pattern[curPatternPos] == multipleWildcard))
                            && !pointTested[0][curInputPos] && !pointTested[1][curPatternPos])
                        {
